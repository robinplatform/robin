...
export function useRpcQuery<R>(options: Omit<UseQueryOptions<R, unknown, R, [string, unknown]>, 'queryKey'> & {
    method: string;
    data: unknown;
    result: z.Schema<R>;
}) {
    return useQuery<R, unknown, R, [string, unknown]>({
        ...
        queryFn: async ({ queryKey }) => {
            ...
            const { data } = await axios.post(`/api/rpc/${rpcMethodName}`, rpcMethodData);
